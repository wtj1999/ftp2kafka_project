...
def compute_cell_dcr(csv_path) -> list:
    ...
    cols = [c for c in df.columns if re.match(r'^CellVolt\d+$', c, re.I)][:n_packs*cells_num]
...
def result_csv_to_json_kelie(
    ...
    cell_cols = [c for c in all_cols if re.match(r'^CellVolt\d+$', c, re.I)][:n_packs*cells_num]
    if not cell_cols:
        cell_cols = [c for c in all_cols if c.startswith("CellVolt")][:n_packs*cells_num]
